package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IAvatarImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(_selectedColorIndex >= var_299.length)
         {
            return "";
         }
         var petColor:IPetColor = var_299[_selectedColorIndex] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_582[var_1484] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         _selectedColorIndex = param1.index;
         if(_selectedColorIndex < 0 || _selectedColorIndex > var_299.length)
         {
      ...
      override public function init() : void
      {
         ...
         var_830 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_299 = [];
         var_582 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_830,this);
               if(_loc11_)
               {
                  var_582 = _loc11_.breeds;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_582.length)
         {
            _loc12_ = var_582[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_830,_loc12_.id),getRaceLocalizationKey(var_830,_loc12_.id)));
            _loc6_++;
         }
         var_579.add(var_830,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_865,onApproveNameResult);
         _loc5_ = var_579.getWithIndex(0);
         if(_loc5_ != null)
         {
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_579.getWithIndex(0),page,p);
      }
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_579.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(_selectedColorIndex >= 0 && _selectedColorIndex < var_299.length)
         {
            _loc7_ = var_299[_selectedColorIndex] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_582[var_1484] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_830,_loc3_.id,_loc2_,AvatarScaleType.const_56,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_262,AvatarAction.const_688);
