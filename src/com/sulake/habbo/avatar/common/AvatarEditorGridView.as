package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridView
   {
      ...
      private var var_887:Boolean = true;
      
      private var var_497:IItemGridWindow;
      ...
      public function AvatarEditorGridView(param1:IAvatarEditorCategoryModel, param2:String, param3:IHabboWindowManager, param4:IAssetLibrary)
      {
         ...
         if(_view != null)
         {
            var_497 = _view.findChildByName("part_thumbs") as IItemGridWindow;
            var_417 = new Array();
            var_417.push(_view.findChildByName("palette0") as IItemGridWindow);
            var_417.push(_view.findChildByName("palette1") as IItemGridWindow);
         }
      }
      ...
      private function paletteEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_55)
         {
            ...
            _loc7_ = 0;
            while(_loc7_ < const_1076)
            {
               if(var_417.length > _loc7_)
               {
                  _loc4_ = var_417[_loc7_] as IItemGridWindow;
                  _loc5_ = _loc4_.getGridItemIndex(_loc6_);
                  if(_loc5_ > -1)
                  {
                     _model.selectColor(var_1255,_loc5_,_loc7_);
                     return;
                  }
               }
               _loc7_++;
      ...
      public function initFromList() : void
      {
         ...
         var _loc5_:* = null;
         var _loc6_:* = null;
         var _loc1_:CategoryData = _model.getCategoryData(var_1255);
         if(!_loc1_)
         {
            return;
         }
         var_497.removeGridItems();
         for each(_loc2_ in _loc1_.parts)
         {
            if(_loc2_)
            {
               var_497.addGridItem(_loc2_.view);
         ...
         _loc3_ = 0;
         while(_loc3_ < const_1076)
         {
            _loc4_ = _loc1_.getPalette(_loc3_);
            _loc5_ = var_417[_loc3_] as IItemGridWindow;
            if(!(!_loc4_ || !_loc5_))
            {
      ...
      {
         var _loc3_:IWindow = var_497.getGridItemAt(param1);
         if(!_loc3_)
         {
            return;
         }
      ...
      private function partEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_55)
         {
            _loc3_ = var_497.getGridItemIndex(param1.window);
            _model.selectPart(var_1255,_loc3_);
         }
      }
