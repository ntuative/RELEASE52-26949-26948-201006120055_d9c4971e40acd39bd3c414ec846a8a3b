package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         _main.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,_data.flatId),_main,WindowTracker.const_661,_data.flatId),var_52,false,false,true);
      }
      ...
      private function act(param1:Boolean) : void
      {
         ...
         var _loc2_:int = determineAction(param1,var_781.isSelected);
         _main.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1307,_loc2_,var_419.text,"",""));
         if(false)
         {
            _main.connection.send(new ModerateRoomMessageComposer(_data.flatId,var_1126.isSelected,var_1125.isSelected,var_781.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1304);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1199);
      }
      ...
      public function populate() : void
      {
         ...
         var_781 = ICheckBoxWindow(var_52.findChildByName("kick_check"));
         var_1126 = ICheckBoxWindow(var_52.findChildByName("lock_check"));
         var_1125 = ICheckBoxWindow(var_52.findChildByName("changename_check"));
