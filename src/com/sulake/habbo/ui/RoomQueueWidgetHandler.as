package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_390:
               ...
               {
                  case RoomSessionQueueEvent.const_853:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_830:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_903) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_903) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1185) + 1;
                  }
               }
               else
               {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRoomQueueMessage.const_714);
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_714:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_830);
               break;
            case RoomWidgetRoomQueueMessage.const_664:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_853);
               break;
            case RoomWidgetRoomQueueMessage.const_754:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_215,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_390];
      }
